...
public interface ProductionPlanRepo extends JpaRepository<ProductionPlanVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT A.docid,B.workorderid,B.vapduedate,A.customername,D.partycode,D.partyname,A.fgqty FROM t_routecardentry A, t_workorder B,t_itemparticulars C,partymaster D WHERE A.wono=B.workorderid AND B.workorderid=c.workorderid AND A.fgpartname=D.partyname\r\n"
	...
	@Query(nativeQuery = true, value = "SELECT DISTINCT B.processname FROM m_itemwiseprocess A,m_itemwiseprocessdetails B WHERE A.itemwiseprocessid=B.itemwiseprocessid AND A.processtype=\"Prodution\" AND A.orgid=?1 AND A.item=?2;")
	Set<Object[]> getProcessName(Long orgId, String item);

	@Query(nativeQuery = true, value = "SELECT CAST(A.machineno AS CHAR(100)) as machineno, A.machinename FROM m_machinemaster A WHERE A.machinename NOT IN (SELECT C.machinename FROM productionplan B JOIN productionplandetails C ON B.productionplanid = C.productionplanid WHERE A.orgid=?1 AND (?2 BETWEEN C.fromdate AND C.todate) OR C.status = 'PENDING') GROUP BY A.machineno, A.machinename HAVING (?3 = 0)\r\n"
			+ " UNION\r\n"
