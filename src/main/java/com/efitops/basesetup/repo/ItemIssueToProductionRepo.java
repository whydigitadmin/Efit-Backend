...
public interface ItemIssueToProductionRepo extends JpaRepository<ItemIssueToProductionVO, Long>{
	...
	@Query(nativeQuery = true,value = "select  a.docid from t_routecardentry a where a.orgid=?1 and upper(status)='PENDING' and active=1 and cancel=0 group by \r\n"
			+ " a.docid order by  a.docid")
	Set<Object[]> findRouteCardEntryNoForItemIssueToProduction(Long orgId);

	
	@Query(nativeQuery = true,value = "select  a.wono,a.fgpartname,a.fgpartdesc,a.fgqty from t_routecardentry a where a.orgid=?1 and a.docid=?2 and active=1 and cancel=0 group by a.wono,a.fgpartname,a.fgpartdesc,a.fgqty order by  a.wono")
	Set<Object[]> findRouteCardEntryDetailsForItemIssueToProduction(Long orgId, String routeCardNo);

	@Query(nativeQuery = true,value = "select  b.itemcode,b.itemdesc,b.uom,b.qty  from m_bom a join m_bomdetails b where a.bomid=b.bomid and a.orgid=?1 and a.productcode=?2 and active=1 and cancel=0 group by  b.itemcode,b.itemdesc,b.uom,b.qty  order by  b.itemcode")
