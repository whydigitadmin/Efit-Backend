...
{
	@Query(nativeQuery = true, value = "select * from  thirdPartyInspection  where orgid=?1")
	...
	@Query(nativeQuery = true, value ="SELECT grnno,inwardno,pono,customer,suppliername FROM efit_ops.grn where cancel=0 and orgid=?1")
