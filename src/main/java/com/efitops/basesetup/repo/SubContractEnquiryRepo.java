...
public interface SubContractEnquiryRepo extends JpaRepository<SubContractEnquiryVO, Long> {
	@Query(nativeQuery = true, value = "select * from  subcontractenquiry  where orgid=?1")
	...
	@Query(nativeQuery = true, value = " select a.item,a.itemdescription,a.process,a.quantity from\r\n"
			+ " issuetosubcontractitemdetails a,issuetosubcontractor a1  where a1.orgid=?1 and a.issuetosubcontractorid=a1.issuetosubcontractorid and \r\n"
	...
	@Query(nativeQuery = true, value = "select a.docid,a.docdate,department from aissuetosubcontractor a where a.orgid=?1 and a.routecardno=?2 and  a.active=true group by docid")
