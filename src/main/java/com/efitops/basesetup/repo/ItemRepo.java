...
public interface ItemRepo extends JpaRepository<ItemVO, Long>{
	...
	@Query(nativeQuery = true, value = "select itemgroup from material where orgid=?1 and materialtype=?2 and active=1")
	Set<Object[]> findMaterialGroupFromMaterialType(Long orgId, String materialType);

	@Query(nativeQuery = true, value = "select a1.itemsubgroup from material a, materialdetail a1 where a.orgid=?1 and a.materialtype=?2 and \r\n"
