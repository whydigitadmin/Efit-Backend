...
public interface PurchaseIndentRepo extends JpaRepository<PurchaseIndentVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT * FROM purchaseindent where orgid=?1")
	...
	@Query(nativeQuery=true,value ="select b.itemcode,b.itemdesc,b.uom,b.qty from bom a join bomdetails b where a.bomid = b.bomid and a.orgid=?1 and a.productcode=?2 and active=1 order by 1" )
	Set<Object[]> findBomItemDetailsForPurchase(Long orgId, String fgPart);

	@Query(nativeQuery=true,value ="SELECT DISTINCT a.docid FROM workorder a  WHERE a.orgid = ?1 AND a.customercode = ?2 AND a.active = 1 AND a.docid NOT IN (SELECT c.workorderno FROM t_purchaseindent c WHERE c.orgid = ?1) ORDER BY a.docid" )
