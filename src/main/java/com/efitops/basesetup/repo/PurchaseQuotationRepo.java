...
public interface PurchaseQuotationRepo extends JpaRepository<PurchaseQuotationVO, Long>{
	...
	@Query(nativeQuery=true,value ="SELECT a.docid,a.docdate,a.suppliername,a.suppliercode FROM t_purchaseenquiry a  WHERE a.orgid = ?1 AND a.customercode = ?2 and a.workorderno=?3 AND a.active = 1 AND a.docid NOT IN (SELECT c.enquiryno FROM t_purchasequotation c WHERE c.orgid = ?1) ORDER BY a.docid" )
	...
	@Query(nativeQuery=true,value ="SELECT DISTINCT a.docid FROM t_workorder a  WHERE a.orgid = ?1 AND a.customercode = ?2 AND a.active = 1 ORDER BY a.docid" )
	Set<Object[]> findWorkOrderNoForPurchaseQuotation(Long orgId, String customerCode);

	@Query(nativeQuery=true,value ="SELECT DISTINCT b.partno, b.partname, b.requiredqty, a.customerpono FROM t_workorder a JOIN t_itemparticulars b ON a.workorderid = b.workorderid WHERE a.orgid = ?1 AND a.docid = ?2 AND a.active = 1 ORDER BY b.partno" )
