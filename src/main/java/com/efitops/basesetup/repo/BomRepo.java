...
public interface BomRepo extends JpaRepository<BomVO, Long>{
	...
	@Query(nativeQuery = true, value = "SELECT itemname,itemdesc,primaryunit FROM efit_ops.m_item where orgid=?1 and itemtype=?2 and cancel =0 and active = 1")
	Set<Object[]> findFGSFGPartDetails(Long orgId,String productType);

	@Query(nativeQuery = true, value = "SELECT itemname,itemdesc,primaryunit,itemtype FROM efit_ops.m_item where orgid=?1 and itemtype in ('Raw Material','SFG')and cancel =0 and active = 1")
