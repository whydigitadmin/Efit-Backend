...
public interface GateInwardEntryRepo extends JpaRepository<GateInwardEntryVO, Long>{
	...
	@Query(nativeQuery = true, value = "select docid from t_purchaseorder where orgid=?1 and suppliercode=?2 and active =1 order by 1")
	Set<Object[]> findPurchaseOrderNoForGateInward(Long orgId, String purchaseIndentNo);

	@Query(nativeQuery = true, value = "select b.item,b.itemdesc,b.uom,b.qty from t_purchaseorder a join t_purchaseorderdetails b ON a.purchaseorderid = b.purchaseorderid where a.orgid=?1 and a.suppliercode=?2 and active =1 order by 1")
