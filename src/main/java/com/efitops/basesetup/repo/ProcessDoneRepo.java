...
public interface ProcessDoneRepo extends JpaRepository<ProcessDoneVO, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT A.docid,A.fgpartname,A.fgpartdesc,A.fgqty FROM t_routecardentry A WHERE A.orgid=?1 AND A.customername=?2 GROUP BY A.docid,A.fgpartname,A.fgpartdesc,A.fgqty;")
	...
	@Query(nativeQuery = true, value = "SELECT  A.departmentname FROM m_department A WHERE A.orgid =?1 ORDER BY A.departmentname;")
